...
#define ql_calc_ratehelpers_hpp
    ...
    SEQSEQ(ANY) SAL_CALL qlFxSwapRateHelper(
        const ANY &ObjectId,
        const ANY &FwdPoint,
        const ANY &SpotFx,
        const ANY &Tenor,
    ...
    SEQSEQ(ANY) SAL_CALL qlRateHelperLatestRelevantDate(
        const ANY &ObjectId,
        const ANY &Trigger) throw(RuntimeException);

    SEQSEQ(ANY) SAL_CALL qlRateHelperMaturityDate(
        const ANY &ObjectId,
        const ANY &Trigger) throw(RuntimeException);

    SEQSEQ(ANY) SAL_CALL qlRateHelperPillarDate(
        const ANY &ObjectId,
        const ANY &Trigger) throw(RuntimeException);
