...
SEQSEQ(ANY) SAL_CALL CalcAddins_impl::qlFraRateHelper(
        ...
        const ANY &IborIndex,
        const ANY &PillarDate,
        const ANY &CustomPillarDate,
        const sal_Int32 Permanent,
        const ANY &Trigger,
        const sal_Int32 Overwrite) throw(RuntimeException) {
    try {
        ...
        std::string PillarDateCpp;
        if(PillarDate.hasValue()) 
            calcToScalar(PillarDateCpp, PillarDate);
        else
            PillarDateCpp = "MaturityDate";

        ObjectHandler::property_t CustomPillarDateCpp;
        calcToScalar(CustomPillarDateCpp, CustomPillarDate);
        ...
        QuantLib::Date CustomPillarDateLib;
        if(!CustomPillarDate.hasValue() and typeid(QuantLib::Date())==typeid(QuantLib::Date())) 
            CustomPillarDateLib = QuantLib::Date();
        else
            calcToScalar(CustomPillarDateLib, CustomPillarDate);
...
SEQSEQ(ANY) SAL_CALL CalcAddins_impl::qlFxSwapRateHelper(
        const ANY &ObjectId,
        const ANY &FwdPoint,
        const ANY &SpotFx,
        const ANY &Tenor,
        ...
        const ANY &Convention,
        const sal_Int32 EndOfMonth,
        const sal_Int32 IsFxBaseCurrencyCollateralCurrency,
        const ANY &CollateralCurve,
        const sal_Int32 Permanent,
        const ANY &Trigger,
        const sal_Int32 Overwrite) throw(RuntimeException) {
    try {
        ...
        std::string FwdPointCpp;
        calcToScalar(FwdPointCpp, FwdPoint);

        std::string SpotFxCpp;
        calcToScalar(SpotFxCpp, SpotFx);
        ...
        bool IsFxBaseCurrencyCollateralCurrencyCpp;
        calcToScalar(IsFxBaseCurrencyCollateralCurrencyCpp, IsFxBaseCurrencyCollateralCurrency);

        std::string CollateralCurveCpp;
        calcToScalar(CollateralCurveCpp, CollateralCurve);
        ...
        OH_GET_OBJECT_DEFAULT(FwdPointCoerce, FwdPointCpp, ObjectHandler::Object)
        QuantLib::Handle<QuantLib::Quote> FwdPointLibObj =
            QuantLibAddin::CoerceHandle<
                QuantLibAddin::Quote,
                QuantLib::Quote>()(
                    FwdPointCoerce, QuantLib::Handle<QuantLib::Quote>());

        OH_GET_OBJECT_DEFAULT(SpotFxCoerce, SpotFxCpp, ObjectHandler::Object)
        QuantLib::Handle<QuantLib::Quote> SpotFxLibObj =
            QuantLibAddin::CoerceHandle<
                QuantLibAddin::Quote,
                QuantLib::Quote>()(
                    SpotFxCoerce, QuantLib::Handle<QuantLib::Quote>());

        OH_GET_OBJECT_DEFAULT(CollateralCurveCoerce, CollateralCurveCpp, ObjectHandler::Object)
        QuantLib::Handle<QuantLib::YieldTermStructure> CollateralCurveLibObj =
            QuantLibAddin::CoerceHandle<
                QuantLibAddin::YieldTermStructure,
                QuantLib::YieldTermStructure>()(
                    CollateralCurveCoerce, QuantLib::Handle<QuantLib::YieldTermStructure>());
        ...
        do { 
            std::ostringstream errorMsg; 
            errorMsg << "ERROR: qlFxSwapRateHelper: " << e.what(); 
            OH_LOG_MESSAGE(errorMsg.str());
...
SEQSEQ(ANY) SAL_CALL CalcAddins_impl::qlRateHelperLatestRelevantDate(
        const ANY &ObjectId,
        const ANY &Trigger) throw(RuntimeException) {
    try {
        ...
        do { 
            std::ostringstream errorMsg; 
            errorMsg << "ERROR: qlRateHelperLatestRelevantDate: " << e.what(); 
            OH_LOG_MESSAGE(errorMsg.str());
...
SEQSEQ(ANY) SAL_CALL CalcAddins_impl::qlRateHelperMaturityDate(
        const ANY &ObjectId,
        const ANY &Trigger) throw(RuntimeException) {
    try {
        ...
        do { 
            std::ostringstream errorMsg; 
            errorMsg << "ERROR: qlRateHelperMaturityDate: " << e.what(); 
            OH_LOG_MESSAGE(errorMsg.str());
...
SEQSEQ(ANY) SAL_CALL CalcAddins_impl::qlRateHelperPillarDate(
        const ANY &ObjectId,
        const ANY &Trigger) throw(RuntimeException) {
    try {
        ...
        do { 
            std::ostringstream errorMsg; 
            errorMsg << "ERROR: qlRateHelperPillarDate: " << e.what(); 
            OH_LOG_MESSAGE(errorMsg.str());
